...
Last modified: Thu Apr 11 11:05:07 2024
"""
...
for wib_ip in LD.WIB_IPs:
    print (wib_ip)
    if "34.11" in wib_ip:
        LD.act_fembs[wib_ip] = [False, False, False, True] #1-1-3-69
    elif "34.12" in wib_ip:
        LD.act_fembs[wib_ip] = [False, False, True, False] #1-2-2-15
    elif "34.13" in wib_ip:
        LD.act_fembs[wib_ip] = [False, False, False, True] #1-3-3-62
    elif "34.15" in wib_ip:
        LD.act_fembs[wib_ip] = [True, False, False, False] #1-5-0-40
    elif "34.16" in wib_ip:
        LD.act_fembs[wib_ip] = [False, True, False, False] #1-6-1-103
    elif "34.21" in wib_ip:
        LD.act_fembs[wib_ip] = [True, False, False, False] #2-1-0-50
    elif "34.24" in wib_ip:
        LD.act_fembs[wib_ip] = [False, True, False, True] #2-4-1-35, 2-4-3-106
    elif "34.44" in wib_ip:
        LD.act_fembs[wib_ip] = [False, True, False, False] #4-4-1-44
    else:
        LD.act_fembs[wib_ip] = [False, False, False, False]
...
if True:
    ...
    Create_Folder()
    cfglog = LD.CE_MON_CFG(pls_cs=0, dac_sel=0, mon_cs=1, sts=0, sg0=0, sg1=1, st0 =1, st1=1, snc=0, monchn=-1)

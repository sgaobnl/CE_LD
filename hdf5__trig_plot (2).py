...
Last modified: 5/9/2025 12:40:40 PM
"""
...
hdf5_fp=rst_dir + "ufemb%d_trigger2.hdf5"%ufemb_id
    ...
    for ch in [chin]:
        ...
        if len(data) > 0:
            ...
            for idx in jumps:
                ...
                for t, d in zip(subt, subd):

                    sub = zip(subt, subd)

                subd_otp = np.where(
                subts.append(subt)
                subds.append(ds[subd)
                prev = idx + 1

...
def get_avg_values(group1, group2):
    """
    Combine datasets from group1 and group2, preferring group1 datasets.
...
def main():
    filename1 = 'femb1_darkrate_vs_time.hdf5'
    filename2 = 'femb2_darkrate_vs_time.hdf5'
    output_dir = 'plots_combined_femb1_femb2_subplots'
    ...
    with h5py.File(filename1, 'r') as f1, h5py.File(filename2, 'r') as f2:
        ...
        for hv_prefix, groups in hv_groups.items():
            ...
            for i, group_name in enumerate(groups):
                ...
                normalized_dcr_all_hours = (avg_values - global_mean) / global_mean
            ...
            fig2.suptitle(f'Normalized DCR per Day for {hv_prefix}', fontsize=16)
            ...
            plt.tight_layout(rect=[0.05, 0.05, 1, 0.95])
            filepath2 = os.path.join(output_dir, f'Normalized_DCR_per_day{hv_prefix}.png')

...
store CN and Gaussian parameters in HDF5 file: CN.hdf5
Uses highest amplitude peak as 1-pe reference with validation.
Sum limited to 0.5 pe < N < 4.5 pe range for CN calculation.
"""
...
drfp = "/data/disk1/koloina/CN/SiPM/CN.hdf5"
        ...
        with h5py.File(hdf5_fp, "r") as f:
            ...
            for ch in range(32):
                ...
                for hi in range(int(tlen // (1e9 * h2s)) + 1):
                    ...
                    # === Validate CN value ===
                    if not np.isfinite(CN) or CN < 0 or CN > 1:

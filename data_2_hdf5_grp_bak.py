...
Last modified: 5/2/2025 4:28:23 PM
"""
    ...
    if os.path.exists(hdf5_fp):
        ...
        with h5py.File(hdf5_fp, 'w') as f:
            ...
            for chi in range(32):
                ...
                dset1 = grp.create_dataset("TimeStamp", shape=(0,), maxshape=maxshape, dtype="i8", chunks=True)
                dset2 = grp.create_dataset("Value", shape=(0,), maxshape=maxshape, dtype="u2", chunks=True)
...
def append_structured_data( hdf5_fp, tdszip,anafp,  dtype=np.dtype([("TS","i8"), ("Value", "u2")]),):
    ...
    with h5py.File(hdf5_fp, 'a') as f:
        for chi in range(32):
            if len(tdszip[chi]) > 0:
                ts,ds = list(zip(*tdszip[chi]))
            else:
                continue
            newdata = np.array(ts, dtype="i8")
            dset = f["/CH%02d/TimeStamp"%chi]
            old_size = dset.shape[0]
            new_size = newdata.shape[0]
            dset.resize((old_size + new_size,))
            dset[old_size:] = newdata

            newdata = np.array(ds, dtype="u2")
            dset = f["/CH%02d/Value"%chi]
            old_size = dset.shape[0]
            new_size = newdata.shape[0]
            dset.resize((old_size + new_size,))
            dset[old_size:] = newdata

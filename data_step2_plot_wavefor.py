...
Last modified: 6/10/2025 9:44:56 AM
"""
...
rootp = "G:/SiPM/Anaata_20250609_00_00"
fn = "uFEMB1_20250609_00_08_00000000000000000262622153148651.tana"    
    ...
    if True:
        if len(datas[chi])> 0:
            ...
            for xi in range(len(datas[chi])):
                if xi == 0:
                    plt.plot((np.arange(len(datas[chi][xi][1])))*0.5, datas[chi][xi][1], label=label)
                else:
                    plt.plot((np.arange(len(datas[chi][xi][1])))*0.5, datas[chi][xi][1])

"""
This script:
- Defines a Gaussian function for peak fitting and a general gaussian envelope fitting function to fit the overall shape of multiple peaks.
- Saves all computed data (fitted peak centers, amplitudes, sigmas, FWHMs, resolutions, SNR, PDE (envelope parameters))into the HDF5 output file, organized by channel and date.
- Peaks are filtered depending on HV group (HV1, HV2, HV3) to define fitting windows.
"""
...
hdf5_output = os.path.join(rootdir, "Trig_Analysis.hdf5")
    ...
    for ufemb_id in [1, 2]:
        ...
        for local_ch in range(32):
            ...
            for xi in range(len(all_subts)):
              ...
              #Defining fitting windows for gaussian
